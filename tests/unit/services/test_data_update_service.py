    ...
    *,
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if (
            ...
            and request.url.path.startswith(f'/Logical-Byte/eer-resource/{commit}/data/v2/')
        ):
...
def test_download_updates_applies_immediately_when_scanner_is_idle(tmp_path):
    ...
    client, _requested_paths = _build_mock_client(
        {
            ...
            'WeaponType.json': (remote_root / 'WeaponType.json').read_text(encoding='utf-8'),
        }
    )
    ...
    assert runtime.get_static_game_data().get_weapon('weapon_1').name == 'Updated Weapon'  # type: ignore[union-attr]
...
def test_download_updates_defers_apply_until_scan_finishes(tmp_path):
    ...
    assert runtime.get_static_game_data().get_weapon('weapon_1').name == 'Bundled Weapon'  # type: ignore[union-attr]
...
def test_download_updates_does_not_replace_existing_override_on_invalid_payload(tmp_path):
    bundled_root = tmp_path / 'bundled'
    override_root = tmp_path / 'override'
    state_path = tmp_path / 'data_update_state.json'
    _write_data_root(bundled_root, weapon_name='Bundled Weapon')
    ...
    client, _requested_paths = _build_mock_client(
        {
            ...
            'WeaponType.json': (bundled_root / 'WeaponType.json').read_text(encoding='utf-8'),
        }
    )
    service = DataUpdateService(
        client=client,
    ...
    assert (override_root / 'Weapon.json').read_text(encoding='utf-8') == existing_weapon_json
