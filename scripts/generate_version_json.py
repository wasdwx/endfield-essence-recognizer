...
def main() -> None:
    ...
    parser.add_argument(
        "--version", type=str, default=None, help="程序版本号，可带 v 前缀"
    )
    parser.add_argument("--download-url", type=str, required=True, help="主下载地址")
    parser.add_argument(
        "--asset-path", type=Path, default=None, help="用于计算 sha256 的本地文件"
    )
    ...
    parser.add_argument(
        "--mirror", action="append", default=[], help="附加镜像，格式为 key=url"
    )
